...
evaluate_data = 'evaluate_mtcnn_MIDV_v5.txt'
...
def main():
    ...
    with open(os.path.join(cbdar_path, evaluate_data), 'r') as f:
        ...
        for line in lines:
            ...
            if pre_num:
                for i in range(pre_num):
                    f_new.write('%d %d %d %d '%(pre_bboxes[i][0],pre_bboxes[i][1],pre_bboxes[i][2],pre_bboxes[i][3]))
                    f_new.write('%f '%(pre_bboxes[i][4]))
            f_new.write('\n')
